...
interface AddItineraryItemModalProps {
    isOpen: boolean;
    onClose: () => void;
    onAddItem: (itemData: Omit<ItineraryItem, 'id'>) => void;
    dayDate?: string;
}

const AddItineraryItemModal: React.FC<AddItineraryItemModalProps> = ({ isOpen, onClose, onAddItem, dayDate }) => {
        ...
        >
            ...
            >
                <header className="flex-shrink-0">
                    ...
                    <h2 className="text-2xl font-bold">새 일정 추가</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-4 mt-6 overflow-y-auto flex-1 pr-2">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label htmlFor="category" className="flex items-center text-sm font-medium text-gray-300 mb-1"><Tag size={14} className="mr-2" />카테고리</label>
                            <select id="category" value={category} onChange={(e) => setCategory(e.target.value as Category)} className="w-full bg-slate-700 rounded-md px-3 py-2 text-white border border-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500 h-10">
                        ...
                        <div>
                            <label htmlFor="time" className="flex items-center text-sm font-medium text-gray-300 mb-1"><Clock size={14} className="mr-2" />시간</label>
                            <input id="time" type="time" value={time} onChange={(e) => setTime(e.target.value)} className="w-full bg-slate-700 rounded-md px-3 py-2 text-white border border-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500 h-10" required />
                        </div>
                    </div>
                    <div>
                        <label htmlFor="title" className="flex items-center text-sm font-medium text-gray-300 mb-1"><Type size={14} className="mr-2" />타이틀</label>
                        <input id="title" type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full bg-slate-700 rounded-md px-3 py-2 text-white placeholder-gray-500 border border-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="예: 시부야 스카이 전망대" required />
                    </div>
                    <div>
                        <label htmlFor="description" className="flex items-center text-sm font-medium text-gray-300 mb-1"><FileText size={14} className="mr-2" />한 줄 설명</label>
                        <textarea id="description" value={description} onChange={(e) => setDescription(e.target.value)} className="w-full bg-slate-700 rounded-md px-3 py-2 text-white placeholder-gray-500 border border-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500 h-16 resize-none" placeholder="예: 미리 티켓 예매하기"></textarea>
                    </div>
                    ...
                    <div>
                        ...
                        {imageSourceType === 'url' ? (
                            ...
                            <input id="images" type="file" multiple accept="image/*" onChange={handleImageChange} className="w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-600 file:text-white hover:file:bg-blue-500"/>
                        )}
                ...
                <div className="flex justify-end pt-6 space-x-3 flex-shrink-0">
                    ...
                    <button type="submit" onClick={handleSubmit} className="px-4 py-2 rounded-md text-white bg-blue-600 hover:bg-blue-500 font-semibold transition-colors">일정 추가</button>
                </div>
            </div>
            <style>{`
                @keyframes fade-in {

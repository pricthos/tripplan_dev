...
interface CreateTripModalProps {
    isOpen: boolean;
    onClose: () => void;
    onAddTrip: (tripData: NewTripData) => void;
}

const CreateTripModal: React.FC<CreateTripModalProps> = ({ isOpen, onClose, onAddTrip }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const addTripWithImage = (imageData: string) => {
            onAddTrip({ title, destination, startDate, endDate, coverImage: imageData });
        };

        if (imageSourceType === 'upload' && coverImageFile) {
            const reader = new FileReader();
            reader.onloadend = () => {
                addTripWithImage(reader.result as string);
            };
            reader.readAsDataURL(coverImageFile);
        } else {
            addTripWithImage(coverImageUrl);
        }
    };
    ...
    return (
        ...
        >
            ...
            >
                ...
                <h2 className="text-2xl font-bold mb-6">새 여행 만들기</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label htmlFor="title" className="block text-sm font-medium text-gray-300 mb-1">제목</label>
                        <input id="title" type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full bg-slate-700 rounded-md px-3 py-2 text-white placeholder-gray-500 border border-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="예: 도쿄 3박 4일 미식 여행" required />
                    </div>
                    
                    <div className="relative">
                        <label htmlFor="destination" className="block text-sm font-medium text-gray-300 mb-1">여행지</label>
                    ...
                    <div>
                        ...
                        {imageSourceType === 'url' ? (
                            <input id="coverImageUrl" type="url" value={coverImageUrl} onChange={handleUrlChange} className="w-full bg-slate-700 rounded-md px-3 py-2 text-white placeholder-gray-500 border border-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="https://example.com/image.jpg" />
                        ) : (
                            <div>
                                <label htmlFor="file-upload" className="relative cursor-pointer bg-slate-700 rounded-lg border-2 border-dashed border-slate-600 hover:border-blue-500 transition-colors flex flex-col items-center justify-center p-6 text-center h-32">
                    ...
                    <div className="flex justify-end pt-4 space-x-3">
                        ...
                        <button type="submit" className="px-4 py-2 rounded-md text-white bg-blue-600 hover:bg-blue-500 font-semibold transition-colors">생성하기</button>
                    </div>
                </form>
            </div>
            <style>{`
